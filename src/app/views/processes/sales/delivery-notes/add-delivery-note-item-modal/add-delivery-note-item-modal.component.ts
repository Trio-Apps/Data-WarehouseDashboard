...
import { AddDeliveryNoteItemRequest, DeliveryNoteItem } from '../../Models/delivery-note-model';
import { SalesService } from '../../Services/sales.service';

@Component({
  selector: 'app-add-delivery-note-item-modal',
   imports: [
    CommonModule,
    ModalModule,
    ButtonModule,
  ...
  templateUrl: './add-delivery-note-item-modal.component.html',
  styleUrl: './add-delivery-note-item-modal.component.scss',
})
export class AddDeliveryNoteItemModalComponent implements OnInit, OnChanges {
  @Input() visible: boolean = false;
  @Input() item: {item: DeliveryNoteItem,SaleId: number} | null = null;
  ...
  onUpdate(): void {
    ...
    const itemData: AddDeliveryNoteItemRequest = {
       salesOrderItemId: formValue.salesOrderItemId,
      quantity: formValue.quantity,
      comment: formValue.comment
    };
     console.log("updating item data",itemData);
    this.returnService.addDeliveryNoteItem(this.item?.SaleId || 0, itemData).subscribe({
      next: (res: any) => {
