...
export class AddReceiptItemComponent implements OnInit {
  ...
  onAddByBarcode(): void {
    ...
    this.receiptService.addReceiptItemByBarcode(this.receiptId, barcode).subscribe({
      next: (res: any) => {
      ...
      error: (err) => {
        console.error('Error adding receipt item by barcode:', err);
        this.saving = false;
  ...
  onAddManually(): void {
    ...
    this.receiptService.addReceiptItemManually(this.receiptId, itemData).subscribe({
      next: (res: any) => {
      ...
      error: (err) => {
        console.error('Error adding receipt item manually:', err);
        this.saving = false;
        const errorMessage = err.error?.message || 'Error adding item. Please try again.';
        this.toastr.error(errorMessage, 'Error');
        this.cdr.detectChanges();
